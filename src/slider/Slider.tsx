...
const Slider = (props) => {
  ...
  useEffect(() => {
      if (didMountRef.current) {
          ...
          if (Number(value._value) !== newValue) {
  ...
  const getValue = (gestureState) => {
      ...
      const location = _previousLeft.current + (isInverted.current ? -movement : movement);
      return getValueForTouch(location);
  };
  React.useEffect(() => {
  ...
  const getMinimumTrackStyles = (thumbStart) => {
      ...
      if (isVertical.current) {
          minimumTrackStyle.height = Animated.add(thumbStart, thumbSizeValue.current.width / 2);
  ...
  const { minimumValue, maximumValue, minimumTrackTintColor, maximumTrackTintColor, thumbTintColor, containerStyle, style, trackStyle, sliderStyle, thumbStyle, thumbProps, debugTouchArea } = props, other = __rest(props, ["minimumValue", "maximumValue", "minimumTrackTintColor", "maximumTrackTintColor", "thumbTintColor", "containerStyle", "style", "trackStyle", "sliderStyle", "thumbStyle", "thumbProps", "debugTouchArea"]);
  ...
  return (
  <View {...other} style={StyleSheet.flatten([
      isVertical.current
...
const styles = StyleSheet.create({
  ...
  slider: {
      borderBottomLeftRadius: TRACK_SIZE / 2,
      borderBottomRightRadius: TRACK_SIZE / 2,
  },
  trackHorizontal: {
      height: TRACK_SIZE,
  },
